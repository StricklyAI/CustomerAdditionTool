...
def preview_and_edit(customers):
    print("\nWould you like to preview the data one entry at a time or all at once?")
    choice = input("Enter 'one' for one entry at a time or 'all' to see all entries at once: ").strip().lower()

    if choice == 'all':
        print("\nThe following data will be saved:")
        for customer in customers:
            print(customer)
        return True  # Return True to indicate the data was displayed
        ...
        print("Invalid choice. Proceeding with one entry at a time.")
        return preview_and_edit(customers)

    return False  # Return False to indicate the data wasn't displayed in bulk

# Confirm save before writing to YAML file
def confirm_save(customers, data_was_displayed):
    if not data_was_displayed:
        print("\nThe following data will be saved:")
        for customer in customers:
            print(customer)
...
def main():
    try:
        ...
        # Preview and edit customers
        data_was_displayed = preview_and_edit(customers)

        # Confirm save only if customers weren't already displayed
        if confirm_save(customers, data_was_displayed):
            yaml_filename = generate_yaml_filename(customers)
            with open(yaml_filename, 'w') as file:
                yaml.dump({'customers': customers}, file)
